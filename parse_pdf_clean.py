...
class Invoices:
	...
	def wesco(invoice,pdf):
		...
		invoice.num  = pdf.pq('LTTextLineHorizontal:in_bbox("%s, %s, %s, %s")' % (left_corner, bottom_corner-30, left_corner+150, bottom_corner)).text()
		return invoice
	...
	def crescent(invoice,pdf):
		invoice.num  = pdf.pq('LTTextLineHorizontal:in_bbox("%s, %s, %s, %s")' % (520, 738, 570, 746)).text()
		invoice.po = pdf.pq('LTTextLineHorizontal:in_bbox("%s, %s, %s, %s")' % (220, 515, 290, 523)).text()
	...
	'HTTP://WWW.WESCO.COM' : wesco,
	'www.apacherentals.com': apache_rentals,
	'www.fishertools.com' : fisher_tools,
	'CED-PHOENIX': arizona_electric,
	'CED - PHOENIX': ced,
	'http: //www.cesco.com/tc': crescent,
	'http://iesupply.billtrust.com': ies_supply}
